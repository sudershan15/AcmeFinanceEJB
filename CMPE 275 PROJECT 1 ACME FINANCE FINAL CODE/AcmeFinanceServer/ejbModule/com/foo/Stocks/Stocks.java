package com.foo.Stocks;
	...
	@NamedQueries({
			...
			@NamedQuery(name = "findStocksByDateRange", query = " SELECT s from Stocks s WHERE s.symbol=:symbol AND s.date between :startDate and :endDate"),
			@NamedQuery(name = "findStocksByDate", query = " SELECT s from Stocks s WHERE s.date = :date"),
			@NamedQuery(name = "findTopStocksByCap", query = " SELECT s from Stocks s where s.date = :date order by s.cap desc limit 5")})
